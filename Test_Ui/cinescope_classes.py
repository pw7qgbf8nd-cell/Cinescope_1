...
class CinescopMainPage(BasePage):
    def __init__(self, page: Page):
        ...
        self.film_page = f"{self.home_url}movies/57747"

        #Локаторы элементов
        self.detaileds_of_film = "a[href='/movies/57747' and text()='Подробнее']"

    def open_movies(self):
        self.open_url(self.film_page)

    def assert_was_redirect_to_film_page(self):
        self.wait_redirect_for_url(self.film_page)

class CinescopFilmPage(CinescopMainPage):
    def __init__(self, page: Page):
        super().__init__(page)

        # локаторы элементов
        self.feedback_input = "[data-qa-id='movie_review_input']"
        self.rate_button = "button[data-qa-id='movie_rating_select']"
        self.choose_rate_button = "button[name='3' and text()='3']"
